...
const isValidUrl = (url) => {
  // i definitely stole this from somewhere but i don't remember where
...
const getHtml = async (url) => {
  ...
  unwantedElements.forEach((selector) => $(selector).remove());
...
const htmlToPdf = (html, filename) => {
  ...
  pdf.create(html, options).toFile(`./output/${filename}.pdf`, (err, res) => {
    if (err) {
